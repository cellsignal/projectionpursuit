...
_PNG_FILENAME = 'png/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_asymmetric.png'
_MATCH_RESULT_FILENAME = 'match_result/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_match_result_asymmetric.csv'
_MISCLASSIFICATION_FILENAME = 'misclassification/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_misclassification_asymmetric.txt'
_RUN_STATS_FILENAME = 'run_stats/' + Path(training_file).stem + '_' + Path(testing_file).stem  +  '_run_stats_asymmetric_with_noise_filtering.txt'
...
def main(unused_argv):
    ...
    for (key, value) in matcher._unmatched_right_by_closest_left_cluster_id.items():
        ...
        if dict_by_left[str(key)] is None:
            ...
            for right_cl in right_list:
                dict_by_right[str(right_cl)] = [key]
        else:
    ...
    for (key, value) in matcher._unmatched_left_by_closest_right_cluster_id.items():
        ...
        right_list = []
        for cl in value:
            if str(cl) not in dict_by_left.keys():
                left_list = [cl]
                right_lists.append(right_list)
                left_lists.append(left_list)
    ...
    filtered_right_lists_int = []

    for i in range(len(right_lists_int)):
        filtered_right_list_int = right_lists_int[i].copy()
        if -1 in filtered_right_list_int:
            filtered_right_list_int.remove(-1)
        if -11 in filtered_right_list_int:
            filtered_right_list_int.remove(-11)
        filtered_right_lists_int.append(filtered_right_list_int)

    print('filtered right lists int: ', filtered_right_lists_int)
    ...
    for filtered_right_list_int in filtered_right_lists_int:
        if len(filtered_right_list_int) == 0:
            right_names.append('no match')
        else:
            right_names.append('+'.join(map(str, filtered_right_list_int)))
    ...
    noise_0 = test_labels[np.where(test_labels < 0)].shape
    print('noise_0', noise_0)

    (H,) = train_labels.shape
    misclassification = 0
    misclassification_array = np.zeros(len(left_lists_int))
    noise_array = np.zeros(len(left_lists_int))
    ...
    for i in range(H):
        ...
        is_cell_misclassified = False
        is_cell_noise = False
        for j in range(len(left_lists_int)):
            left_list_int = left_lists_int[j]
            right_list_int = right_lists_int[j]
            if test_label >= 0:
                if (train_label in left_list_int) and not (test_label in right_list_int):
                    misclassification += 1
                    misclassification_array[j] += 1
                    is_cell_misclassified = True
            elif train_label in left_list_int:
                noise += 1
                noise_array[j] += 1
                is_cell_noise = True
        misclassification_cell_list.append(is_cell_misclassified)
        noise_cell_list.append(is_cell_noise)
    print(misclassification_array)
    print('misclassified total: {0} of {1}'.format(misclassification, H))
    print('noise: {0} of {1}'.format(noise, H))
    for j in range(len(left_lists_int)):
    ...
    if misclassification > 0:
        ...
        color_shift += 1
    if noise > 0:
        left_names.insert(1, 'noise')
        right_names.insert(1, 'noise')
        color_shift += 1
    ...
    for j in range(len(left_lists_int)):
        ...
        if H_j < _FILTER_SIZE:
            misclassification_array[j] = -1
            H_skip += H_j
            misclassification -= misclassification_j
    ...
    with open(_MATCH_RESULT_FILENAME, 'w') as match_result_file:
        match_result_file.write(
            "left,right_fact,right_must_be,is_misclassified\n")
        for k in range(H):
            c_test = test_labels[k]
            c_train = train_labels[k]
            is_cell_misclassified = misclassification_cell_list[k]
            is_cell_noise = noise_cell_list[k]
            is_misclassified_by_mistake = False
            done_train = 0
            done_test = 0
            left_name = ''
            right_name = ''
            for i_r_l in range(len(right_lists_int)):
                if done_test == 0:
                    right_list_int = right_lists_int[i_r_l]
                    filtered_right_list_int = filtered_right_lists_int[i_r_l]
                    for cl in right_list_int:
                        if cl >= 0 and len(filtered_right_list_int) == 0:
                            is_cell_misclassified = True
                            color_test = 0
                            ...
                            right_name = right_names[i + color_shift]
                ...
                if done_train == 0:
                    for cl in left_list_int:
                        if c_train == cl:
                            ...
                            left_name = left_names[i + color_shift]
                            right_name_must_be = right_names[i + color_shift]
                            if is_cell_misclassified:
                                color_train = 0
                            elif is_cell_noise:
                                color_train = 1
                            else:
                                color_train = i + color_shift
                            if is_misclassified_by_mistake:
                                if misclassification_array[i] > 0:
                                    misclassification -= 1
                                    misclassification_array[i] -= 1
                                else:
                                    lost_by_filter += 1
                                noise += 1
                            colors_train.append(color_train)
                            done_train += 1
            match_result_file.write("{0},{1},{2},{3}\n".format(left_name, right_name, right_name_must_be, is_cell_misclassified))
    match_result_file.close()

    print('lost by filter: ', lost_by_filter)

    misclassification_assert = 0
    noise_assert = 0

    with open(_MISCLASSIFICATION_FILENAME, 'w') as the_file:
        H_actual = H - H_skip
        percent_total = 100.0 * misclassification / H_actual
        percent_noise = 100.0 * noise / H_actual
        the_file.write("misclassification: {0} of {1}, {2}%\n".format(misclassification, H_actual, percent_total))
        for j in range(len(left_lists_int)):
            H_j = H_array[j]
            misclassification_j = misclassification_array[j]
            if misclassification_j >= 0:
                misclassification_assert += misclassification_j
                percent_j = 100.0 * misclassification_j / H_j
                the_file.write('{0} | {1}: {2} of {3}, {4}%\n'.format(left_lists_int[j], filtered_right_lists_int[j],
                                                                      misclassification_array[j], H_j, percent_j))
            else:
                the_file.write('{0} | {1}: {2} of {3}, {4}%\n'.format(left_lists_int[j], filtered_right_lists_int[j],
                                                                      'null', H_j, 'null'))
        the_file.write("\nnoise: {0} of {1}, {2}%\n".format(noise, H, percent_noise))
        for j in range(len(left_lists_int)):
            noise_assert += noise_array[j]
            H_j = H_array[j]
            percent_j = 100.0 * noise_array[j] / H_j
            the_file.write('{0} | {1}: {2} of {3}, {4}%\n'.format(left_lists_int[j], filtered_right_lists_int[j],
                                                                      noise_array[j], H_j, percent_j))

    the_file.close()

    print('misclassification assert', misclassification, misclassification_assert)
    print('noise assert', noise, noise_assert)
    ...
    if 'misclassified' in left_names:
        misclassified_index = left_names.index('misclassified')
        black = np.array([0, 0, 0, 1])
        newcolors[misclassified_index, :] = black
    if 'noise' in left_names:
        noise_index = left_names.index('noise')
        gray = np.array([0.7, 0.7, 0.7, 0.2])
        newcolors[noise_index, :] = gray
    ...
    sc = ax.scatter(*train_data.T, s=0.03, c=colors_train, cmap=newcmp, alpha=1.0)
    ...
    sc = ax.scatter(*test_data.T, s=0.03, c=colors_test, cmap=newcmp, alpha=1.0)
    plt.setp(ax, xticks=[x_min, x_max, (x_max - x_min) / 4000], yticks=[y_min, y_max, (y_max - y_min) / 4000])
