...
_PNG_FILENAME = 'png/' + fn_in + '_' + left + right + 'match_asymmetric_no_black.png'
_MATCH_RESULT_FILENAME = 'match_result/' + fn_in + '_' + left + right + 'match_result_asymmetric_no_black.csv'
_MISCLASSIFICATION_FILENAME = 'misclassification/' + fn_in + '_' + left + right + 'misclassification_asymmetric_no_black.txt'
...
def main(unused_argv):
    ...
    with open(_MATCH_RESULT_FILENAME, 'w') as match_result_file:
        ...
        for k in range(H):
            ...
            if done_train == 0:
                # print(c, 'right no match')
                colors_train.append(len(left_lists))
            match_result_file.write(
