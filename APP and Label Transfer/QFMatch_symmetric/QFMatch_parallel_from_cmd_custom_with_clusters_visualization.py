...
Further  visualizes the matching outcomes and computes the misclassification
rate (if this task is required). Data points that are misclassified
are visually highlighted by being painted black.
Also visualizes the original clusters.
"""
...
_PNG_FILENAME = 'png/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_match.png'
_PNG_CLUSTER_FILENAME = 'png/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_clusters.png'
_MATCH_RESULT_FILENAME = 'match_result/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_match_result.csv'
_MISCLASSIFICATION_FILENAME = 'misclassification/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_misclassification.txt'
_RUN_STATS_FILENAME = 'run_stats/' + Path(training_file).stem + '_' + Path(testing_file).stem + '_run_stats.txt'
...
def main(unused_argv):
    ...
    train_data_2 = np.copy(train_data)
    test_data_2 = np.copy(test_data)
    ...
    train_labels_un = np.unique(train_labels)
    train_labels_un.sort()
    colors_train_2 = []
    for l in train_labels:
        c = train_labels_un.tolist().index(l)
        colors_train_2.append(c)
    colors_train_2 = np.array(colors_train_2)
    ...
    test_labels_un = np.unique(test_labels)
    test_labels_un.sort()
    colors_test_2 = []
    for l in test_labels:
        c = test_labels_un.tolist().index(l)
        colors_test_2.append(c)
    colors_test_2 = np.array(colors_test_2)
    ...
    # plt.tight_layout()
    plt.savefig(_PNG_CLUSTER_FILENAME, dpi=320)
