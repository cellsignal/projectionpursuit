...
for fn_in_test in fn_in_test_list:
    ...
    fn_out_test = 'dml/' + Path(fn_in_train).stem + '.csv' + '_' + Path(fn_in_test).stem + '.csv' + '_hdbscan_with_umap.csv'
    ...
    labels_test = hdbscan.HDBSCAN(core_dist_n_jobs=1, min_samples=min_samples, min_cluster_size=min_cluster_size).fit_predict(embedding_test)
    t5 = time.time()
    print('testing hdbscan done ', fn_in_test, t5 - t4)
