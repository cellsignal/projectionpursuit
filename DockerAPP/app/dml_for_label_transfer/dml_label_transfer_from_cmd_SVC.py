...
clf.fit(embedding_train, labels_train)

for fn_in_test in fn_in_test_list:
    print('test: ', fn_in_test)

    fn_out_test = 'dml/' + Path(fn_in_train).stem + '.csv' + '_' + Path(fn_in_test).stem + '.csv' + '_SVC_with_umap.csv'
    ...
    labels_test = clf.predict(embedding_test)
    t5 = time.time()
